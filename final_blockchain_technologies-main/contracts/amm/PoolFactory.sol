...
contract PoolFactory {
    ...
    function createPool(address tokenA, address tokenB) external returns (address pool) {
        pool = address(new AMMPool(tokenA, tokenB, msg.sender));
    ...
    function createPoolDeterministic(address tokenA, address tokenB, bytes32 salt) external returns (address pool) {
        pool = address(new AMMPool{salt: salt}(tokenA, tokenB, msg.sender));
        emit PoolCreated(pool, tokenA, tokenB, salt);
    }

    function predictDeterministicAddress(address tokenA, address tokenB, address creator, bytes32 salt)
        external
        view
        returns (address predicted)
    {
        bytes memory bytecode = abi.encodePacked(type(AMMPool).creationCode, abi.encode(tokenA, tokenB, creator));
